...
pattern_full = re.compile(r'(\s*// 逐輪（第1節、第2節…）找共同可用時段.*?break;\s*\n\s*\})', re.DOTALL)
match_full = pattern_full.search(code)

if match_full:
    print('Matched FULL block to replace!')
    original_text = match_full.group(1)
...
'''
    ...
    print('Could not find the original loop block.')
